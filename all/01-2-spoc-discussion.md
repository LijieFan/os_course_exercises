...
- 请总结你认为操作系统应该具有的特征有什么？并对其特征进行简要阐述。
  ...
  答：（1）并发性：一段时间内有多道程序在同时运行。（2）虚拟性：将物理实体变成若干个逻辑上的对应物。（3）异步性：在多道程序设计环境下，允许多个进程并发执行。（4）共享性：系统中的资源可供内存中多个并发执行的进程共同使用。（5）持久性。

- 为什么现在的操作系统基本上用C语言来实现？为什么没有人用python，java来实现操作系统？

  答：C语言足够高级并且足够低层。C语言能够容易编写操作系统。同时程序员可以使用C语言方便的描述硬件细节。编译器可以生成高效机器代码，保证了运行的性能。
  Python和Java高级但是不够底层。虽然编写代码较为方便，但是无法很好地接触硬件，性能难以得到保证。
